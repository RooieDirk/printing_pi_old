    ...
    double& outLat, double& outLon)
{
    ...
    DistanceBearingMercator_Plugin(
        nextLat, nextLon, stepLat, stepLon, &myBrg, &myDist);
...
wxString Dlg::createMWVTSentence(
...
wxString Dlg::createMWVASentence(
    double spd, double hdg, double winddirection, double windspeed)
{
    /*
    1    2   3   4        5
...
bool Dlg::GetGribSpdDir(
    wxDateTime dt, double lat, double lon, double& spd, double& dir)
{
                ...
                if (!strcmp(e->Value(), "TWA")) {
                    ...
                    if (angleOut
                        ...
                        for (TiXmlElement* g = e->FirstChildElement(); g;
                            ...
                            if (!strcmp(g->Value(), "SPD")
                                && windspeed > myWindSpeed) {
                                myWindSpeed
                                    = AttributeDouble(g, "WindSpeed", NAN);
                                ...
                                prevSpeed
                                    = myWindSpeed; // attribute for wind speed
                    ...
                    if (!strcmp(f->Value(), "rtept")) {
                        ...
                        for (TiXmlElement* i = f->FirstChildElement(); i;
                            ...
                            if (!strcmp(i->Value(), "extensions")) {
                                ...
                                for (TiXmlElement* j = i->FirstChildElement();
    ...
    : wxDialog(parent, id, title, position, size, style)
{
    ...
    dialogText = new wxListView(this, wxID_ANY, p, sz,
        wxLC_NO_HEADER | wxLC_REPORT | wxLC_SINGLE_SEL, wxDefaultValidator,
        wxT(""));
