...
void ChartBorderCalc::DrawBorders() {
  ...
  if(BorderStyle & (TOPBORDER | BOTTOMBORDER))
    DrawLongitudeScale();
  if(BorderStyle & (LEFTBORDER | RIGHTBORDER))
    DrawLatitudeScale();
}
...
void ChartBorderCalc::DrawLongitudeScale() {
  ...
  double w_min = std::max( (ChR1.GetTop() - ChR2.GetTop()),
                           (ChR2.GetBottom() - ChR1.GetBottom()))* LonDegPPix;
  ...
  while (lon <= LonChartMax) {
    if (IsMultiOf(lon, d[i_major_max])) {
      ...
      if (!minutes) {
        ...
        if(BorderStyle & BOTTOMBORDER)
           w = PrintLabel(wxPoint(LonToPix(lon) + ChRect.GetLeft(), ChR35.GetBottom()),
...
void ChartBorderCalc::DrawLatitudeScale() {
  ...
  double minor_max_even;
  int i_minor_min, i_minor_max, i_major_min, i_major_max;
  double w_min = std::max( (ChR1.GetLeft() - ChR2.GetLeft()),
                           (ChR2.GetRight() - ChR1.GetRight()))* LatDegPPix;
